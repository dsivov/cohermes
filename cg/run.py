...
internals.
    ...
    cohermes-agent "add a gitlab review source"          # interactive, pre-briefed
...
def launch(task: str, print_mode: bool = False, passthrough: list[str] | None = None) -> int:
    ...
    # 2) orient from the shared brain
    print(f"[cohermes-agent] orienting on {task!r} in workspace '{config.WORKSPACE}' …",
          file=sys.stderr)
    brief = orient.brief(task)
...
def main():
    ...
    ap.add_argument("task", help="the task/topic to orient on")
    ap.add_argument("-p", "--print", action="store_true", dest="print_mode",
                    help="one-shot print mode (claude -p)")
